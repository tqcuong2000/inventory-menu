        ...
        return Codec.PASSTHROUGH.flatXmap(
            dyn -> {
                ...
                return DataResult.unbox(
                    DataResult.instance().apply2(combiner, aRes, bRes)
                );
            },
            c -> {
                JsonElement aDyn = aCodec
                    .encodeStart(JsonOps.INSTANCE, aExtractor.apply(c))
                    .result()
                    .orElseThrow();
                JsonElement bDyn = bCodec
                    .encodeStart(JsonOps.INSTANCE, bExtractor.apply(c))
                    .result()
                    .orElseThrow();
                if (
                    aDyn instanceof JsonObject objA &&
                    bDyn instanceof JsonObject objB
                ) {
                    objB
                        .entrySet()
                        .forEach(e -> objA.add(e.getKey(), e.getValue()));
                    return DataResult.success(
                        new Dynamic<>(JsonOps.INSTANCE, objA)
                    );
                }
